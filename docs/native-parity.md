...
| Languages | Nine YAML catalogues, 1,723 messages each, checked for completeness/interpolation/references. Live language switching and Persian text direction without mirrored geometry verified. | Native-speaker review of every translated sentence. |
...
| Troop previews | 20 verified classic stationary poses extracted locally, mounted layers composed with signed origins. Cached AIC allocation, one representative per tile, visible-step support heights and stable bounded draw commands. | Engineer/DE idle poses are unverified and retain explicit rally markers. |
