...
try {
  ...
  if (options['stage-only']) {
    ...
    if (options.published) {
      const current = await until(async () => {const value = await client.evaluate('electronAPI.checkReleaseUpdate(true)'); return value.status === 'current' && value;});
      assert.equal(current.key, key); report.current = current; report.liveCurrentVerified = true;
