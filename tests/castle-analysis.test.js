...
test('full-map atlas elevations anchor sprites and picking to the same map tile at every rotation', () => {
  ...
  const heightFunction = source.slice(source.indexOf('function bodenHoehe'),source.indexOf('function gameMap'));
  // Die Umrechnung Anzeigefeld -> Kartenfeld steht seit dem 17.09.2026 nur noch
  // in iso-geometry.js; die Hoehe nimmt genau dieselbe.
  assert.match(heightFunction,/geo\.mapTileHeight\(gx, gy, currentKeep\(\), atlas\.hoehen, viewRotation\(\)\)/);
});
